...
// Bulk fetch feedback for multiple interviews
export async function getFeedbackForInterviews(interviewIds, userId) {
  if (!userId || !interviewIds?.length) return {};
  
  try {
    const feedbackPromises = interviewIds.map(id => 
      getFeedbackByInterviewId({ interviewId: id, userId })
    );
    ...
    interviewIds.forEach((id, index) => {
      if (feedbackResults[index]) {
        feedbackMap[id] = feedbackResults[index];
      }
    });
    
    return feedbackMap;
  } catch (error) {
    console.error('Error fetching bulk feedback:', error);
    return {};
  }
}
